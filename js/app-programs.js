...
async function generatePhasePeriodization(phaseId, programId) {
  ...
  if (!confirm(`Generate weeks 2–${phase.duration_weeks} from Week 1? This replaces any previously generated weeks for this phase.`)) return

  // Idempotent regeneration — clear any weeks generated by a previous run first (master rows + any already-propagated client copies)
...
async function loadAllPhaseWorkouts(phases) {
  ...
  for (const ph of phases) {
    ...
    const byWeek = {}
    pws.forEach(pw => { (byWeek[pw.week_number] = byWeek[pw.week_number] || []).push(pw) })
    const weekNums = Object.keys(byWeek).map(Number).sort((a, b) => a - b)
    const showWeeks = weekNums.length > 1

    const renderDayGrid = wks => {
      const byDay = {}
      wks.forEach(pw => { (byDay[pw.day_of_week] = byDay[pw.day_of_week] || []).push(pw) })
          ...
          <div style="background:var(--surface-2);border-radius:8px;padding:8px 12px;min-width:120px">
            ...
            ${wks.map((w) => `
              <div style="display:flex;align-items:center;gap:6px;margin-top:4px">
                ...
                ${w.tier ? `<span style="font-size:9px;font-weight:700;color:${tierColor[w.tier]};text-transform:uppercase">${w.tier[0]}</span>` : ''}
                ...
                ${w.week_number === 1 ? `<button onclick="removePhaseWorkout('${w.id}','${ph.id}')" style="font-size:11px;color:var(--text-muted);background:none;border:none;cursor:pointer;padding:0">✕</button>` : ''}
              </div>`).join('')}
          </div>`).join('')}
      </div>`
    }

    el.innerHTML = weekNums.map(w => {
      if (w === 1) return renderEditableWeek1Grid(ph, byWeek[1] || [])
      return `<div style="margin-bottom:10px">
        <div style="font-size:11px;font-weight:700;color:var(--accent);margin-bottom:6px">WEEK ${w}</div>
        ${renderDayGrid(byWeek[w])}
      </div>`
    }).join('')
  }
}

// Editable Week 1 strip — replaces the old "+ Assign workout" modal. Each day has its own
// always-visible native search + select; picking a template assigns it immediately, no modal,
// no separate day/session step. Weeks 2+ (periodization-generated) stay read-only via renderDayGrid above.
function renderEditableWeek1Grid(phase, sessions) {
  ...
  return `
    <div style="font-size:11px;font-weight:700;color:var(--accent);margin-bottom:6px">WEEK 1</div>
...
async function _quickAssignPhaseWorkout(selectEl) {
  ...
  if (value === '__new__') {
    window._phaseWorkoutContext = { phaseId, dayOfWeek, programId: window._openProgramId || null }
    selectEl.value = ''
  ...
  const { data: existing } = await db.from('program_phase_workouts').select('id').eq('phase_id', phaseId).eq('week_number', 1).eq('day_of_week', dayOfWeek).eq('session_order', sessionOrder)
  if (existing?.length) {
  ...
  const { error } = await db.from('program_phase_workouts').insert({
    phase_id: phaseId, day_of_week: dayOfWeek, day_label: dayLabels[dayOfWeek], template_id: value, session_order: sessionOrder, week_number: 1
  })
